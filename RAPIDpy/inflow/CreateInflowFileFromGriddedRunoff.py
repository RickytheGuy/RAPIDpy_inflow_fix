...
class CreateInflowFileFromGriddedRunoff(object):
    ...
    def generateOutputInflowFile(self,
        ...
        if loop_index is not None:
            ...
            directory, file_name = os.path.split(out_nc)
            ...
            out_nc = os.path.join(directory, new_file_name)
    ...
    def generateOutputInflowFile2(self,
                                    out_nc,
                                    start_datetime_utc,
                                    number_of_timesteps,
                                    simulation_time_step_seconds,
        ...
        print("Generating inflow file ...")
        #data_out_nc = Dataset(out_nc, "w", format="NETCDF3_CLASSIC")
        data_out_nc = Dataset('in_memory_dataset', mode='w', memory=True, diskless=True) 
        #data_out_nc = Dataset(out_nc, 'w', parallel = True)
        rivid_list = np.loadtxt(in_rapid_connect_file,
                                delimiter=",",
                                ndmin=1,
